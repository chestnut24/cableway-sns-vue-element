...
@Component({
  selector: 'app-top-station-temperature',
  templateUrl: './top-station-temperature.component.html',
  styleUrls: ['./top-station-temperature.component.scss']
})
export class TopStationTemperatureComponent implements OnInit {
  flow = true;
  pump = true;
  fan = true;
  speedTemp = null;
  ...
  get slowSpeed() {
    return this.mqttService.dataDictionary['上站减速机'] ? this.mqttService.dataDictionary['上站减速机'] : {};
  }
  get Monitor() {
    return this.mqttService.dataDictionary['上站电机温度'] ? this.mqttService.dataDictionary['上站电机温度'] : {};
  }
  ...
  ngOnInit() {
    this.mqttService.dataChange.subscribe( () => {
      if (Object.keys(this.mqttService.dataDictionary).length !== 0) {
        ...
        console.log('test', this.mqttService.dataDictionary['上站1#主电机']);
